...
def PreSplitDataManipulation(dataset, datasetDescription):
	...
	# Hidden missing values - check the zeroes 
	SpruceFeatureCheckListForZeroValues = ['Elevation','Slope','Horizontal_Distance_To_Hydrology','Vertical_Distance_To_Hydrology',
																'Horizontal_Distance_To_Roadways','Horizontal_Distance_To_Fire_Points']
	print("\n\t# Rows in {1} dataframe {0}".format(len(final_data), datasetDescription))
	# Check if any of the first six colums in the Spruce dataset have a zero value
...
def CreateLableAndFeatureSet(final_data, dataDescription):
	...
	# Check distribution of Spruce and Other Tree types cleaned dataset
...
def TuneRandomForestAlgorithm(X_train, Y_train):
	...
	This code was run to determine the optimum parameters for the RandomForest algorithm
	I took a considerable amount of time to execute but he results are described in 
	our analysis report.
	...
	# We check all scoring options in the FOR LOOP below
	scoreOptions = ['accuracy','recall','precision','f1']
	# We create a broade range of option for the possible final 'n_estimator' value
	grid_param = {'n_estimators': [100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700]}


	print("\n\tRunning Grid Search Cross Validation tuning..")

	# Check all scoring options
	# We are looking to minimise False Negatives and hence are primarily interested in the values
	# for the 'recall' option.
	# However finding the other 'n_estimator' suggested parameters will allow us to carry out some
...
def ImplementTunedRandomForestAlgorithm(X_train, X_test, Y_train, Y_test, best_estimator, X):
	...
	# Run 'predict' function to create a matrix of values to compare agains the 'real world' data
	# in the Test set
